...
point the suite's `deleteMany()` wipes at a scratch DB or turn logging up. No **schema-backed**
variable reaches the suite from your shell; the `process.env` reads that bypass `lib/env.ts`
([§19](#19-known-gaps--rules-not-enforced-yet)) are not covered. Nothing re-scrubs between cases in
a file, so a test that mutates `process.env` cleans up after itself; one that needs a specific value
sets it and calls `vi.resetModules()` to clear `env()`'s memoized parse. Playwright reuses an
existing dev server if one is up and needs `tests/e2e/.auth/admin.json` from its global setup.
