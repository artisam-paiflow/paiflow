...
type Cohort = { cohort: string; windowStart: string; testers: Tester[] };
  ...
  if (missing.length > 0) {
    throw new Error(
      `The alpha-tester identities are not populated yet: ${missing
        .map((t) => t.label)
        .join(", ")} still have a null userId in ${path}.\n` +
  ...
  const snapshot = {
    ...
    metrics: {
      ...
      testersActive: {
        ...
        note: "Below the cohort size means a tester issued an account never ran a session.",
      },
    },
    ...(hasFlag("--per-tester")
      ? {
