...
const DEFAULT_SOURCE = "Read-only SQL against the application database selected by DATABASE_URL.";
...
async function main() {
  ...
  // The snapshot is committed as evidence, so it must not claim a provenance the
  // run cannot support: DATABASE_URL decides which database is read, and only the
  // operator knows what that is. The default says exactly that.
  ...
  const snapshot = {
    ...
    source: `${source} Window: rows created on or after ${windowStart.toISOString()}, Stellar testnet. Generated by scripts/instawards-metrics.ts.`,
    metrics: {
      uniqueFlowsDeployed: {
        value: deployments,
        target: "≥ 5",
